...
check_agreement = pandas.read_csv('../input/check_agreement.csv')
check_correlation = pandas.read_csv('../input/check_correlation.csv')
...
#train_eval_probs = 0.4*rf.predict_proba(train_eval[features])[:,1] + 0.3*gbm.predict(xgb.DMatrix(train_eval[features])) + 0.3*rf1.predict_proba(train_eval[features])[:,1]
...
test_probs = 0.38*rf.predict_proba(test[features])[:,1] + 0.22*gbm.predict(xgb.DMatrix(test[features])) + 0.4*rf.predict_proba(test[features])[:,1] 
